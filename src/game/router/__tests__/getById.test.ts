...
describe("Given the GET /games/:id endpoint", () => {
  ...
  describe("When receives a request with id '123456789123456789123456' inexistent", () => {
    test("Then it should response with status 404 and message: 'Failed finding game'", async () => {
      const response = await request(app)
        .get("/games/123456789123456789123456")
        .expect(404);

      expect(response.body).toMatchObject({ message: "Failed finding game" });
    });
  });
